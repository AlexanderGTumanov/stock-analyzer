...
def forecast(model: torch.nn.Module, series: pd.Series, scaler: StandardScaler):
    if len(series) != model.window:
        print('The length of the provided series does not matchthe  model\'s window.')
        return
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("mps") if torch.backends.mps.is_available() else torch.device("cpu")
    model.to(device)
    model.eval()
...
def plot_series(series: pd.Series, model: torch.nn.Module, fit_series: pd.Series, scaler: StandardScaler, end_of_training = None, k: int = 2):
    if len(fit_series) != model.window:
        print('The length of the series to fit does not match the model\'s window.')
        return
    index = series.index
    plt.figure(figsize = (10, 5))
    model_index = fit_series.index
    ...
    mean, std = forecast(model, fit_series, scaler)
